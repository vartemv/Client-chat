...
//int auth_to_server(sockaddr_in server_address, int client_socket);
...
typedef struct AuthPackets: public Packets{
    ...
    std::string DisplayName;
    std::string Secret;
    AuthPackets(uint8_t type , uint16_t id, std::string u_n, std::string disp_name, std::string sec): Packets(type, id){
    ...
    int construct_message(uint8_t *b) override{
        ...
        b += Secret.length() + 1;
        return sizeof(this->MessageType)+sizeof(ID)+Username.length() + 1+DisplayName.length() + 1+Secret.length() + 1;
    }
